 ...
 * decoding-default half is in converters.spec.ts next door.
 *
 * In the Node tier despite being about IndexedDB. The browser tier replaces
 * the page's IndexedDB with `fake-indexeddb` anyway, so running this in
 * Chromium was paying for a browser and then not using it. What tests the
 * *real* store is the e2e tier, against the production build.
 */
