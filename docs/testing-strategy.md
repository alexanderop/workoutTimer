...
6. **Asserting an import boundary, a dependency rule, or a convention no runtime check can see?** → `arch` (`src/__tests__/architecture/`). Three things live there: ArchUnitTS rules over the real module graph; `boundaries.test.ts`, which feeds ESLint deliberate violations; and the text-level tripwires (`uiPrimitives`, `touchConventions`, `viewLogic`, `atomPlacement`), which scan the source for a shape rather than a behavior. The second exists because ArchUnitTS does not parse `.vue` files and because "the codebase has no violations" also passes when nothing is being enforced — the actual `.vue` coverage comes from `no-restricted-imports` in `eslint.config.ts`. The third is for rules the compiler structurally cannot hold: nothing type-checks `['amrap', 'forTime']` against `TimerMode`, and nothing makes a screen's fifth copy of a `switch` fail. A tripwire catches *coverage*, not correctness, and every helper in one is exercised against a synthetic violation as well as the real tree — a rule that only ever sees passing input is not a rule.
