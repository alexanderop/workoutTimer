...
- **DB**: all access via `src/db/index.ts` repositories. The store is at schema v1, so `src/db/schema.ts` has a single `version(1).stores({...})` and no `upgrade()` yet. Changing an existing table means a version bump **plus** a read-path that still accepts the old shape — [local-first.md](local-first.md) for why both are needed and neither is enough.
