...
| [Evidence, not assertion](anti-slop.md) | A lint error from an `anti-slop` rule, or writing anything that parses input, asserts a type, or reads a key off a record |
...
- **Evidence, not assertion.** A type is a claim, and the vendored [anti-slop](https://github.com/dmmulroy/anti-slop) oxlint rules reject the ones nobody checked: `unknown` in a contract, a type assertion with no `// SAFETY:` justification, `typeof` narrowing where a domain match belongs, an annotation that discards a known literal, a key omitted by spreading `{}`. Every generic rule is on at `error`, plus the opt-in `anti-slop-effect` group. The plugin is vendored, not depended on — `tools/oxlint/anti-slop/` is a verbatim upstream copy, held out of prettier, eslint and oxlint so a re-vendor stays a clean diff. What each rule rejects, the shape to write instead, and how to re-vendor: [anti-slop.md](anti-slop.md).
