...
- **Evidence, not assertion — enforced by the vendored [anti-slop](https://github.com/dmmulroy/anti-slop) oxlint rules** (`tools/oxlint/anti-slop/`, every rule at `error` in `.oxlintrc.json`, plus the opt-in `anti-slop-effect` group because Effect is a direct dependency). What they reject, and what to write instead:
  - **`unknown` is not a contract.** A parameter, a return, or a dictionary value typed `unknown` is a boundary that was never parsed. Name the type the caller actually has: `Schema.Json` for a `JSON.parse` result (`src/lib/backupFile.ts` → `src/db/backup.ts`), `Pick<…>` for the two members a poll touches (`src/lib/swUpdateCheck.ts`), a type parameter taken from the decoder for a seam that only re-tags a failure (`decodeRow` in `src/db/repositories/shared.ts`).
  - **A type assertion states an invariant or it does not compile.** Every non-const `as` needs a `// SAFETY:` comment immediately before the assertion or its containing statement, saying what was checked. Most of them turned out not to be needed: `COLOR_SCHEMES.find(…)` beats `includes(…)` plus a cast (`src/state/theme.ts`), `SUPPORTED_LOCALES.some(…)` beats a widening cast (`src/state/locale.ts`), and one documented assertion behind a named helper beats seven at the call sites (`controlValue` in `src/lib/formControl.ts`). The comment must attach to a statement the rule can see — an `export const` hides it, so put it on the initializer.
  - **`typeof` is a representation, not a domain.** Match the value against the vocabulary that owns it — `toRouteName` against `RouteNames`, `parseTimerMode` against `TIMER_MODES`, `Schema.is(…)` in `src/__tests__/helpers/messageAt.ts`. For "does this API exist here", ask `'x' in globalThis` or `x instanceof Function`, which is the question rather than its proxy.
  - **A known value stays known.** `satisfies Readonly<Record<TimerMode, …>>` instead of an annotation that throws the literal keys away (`DEFAULT_CONFIGS`, `MODE_FIELDS`, `RUN_PHASE_WORDS`), and an inferred return type on a helper that returns an anonymous object.
  - **An omitted key is omitted, not spread from `{}`.** `...(x === undefined ? {} : { x })` becomes two branches building the object (`createSession` in `src/db/repositories/sessions.ts`, `toTimerConfig` in `src/features/timer/setupForm.ts`) — which is also what `Schema.optionalKey` means: absent, never present-and-undefined.
  - The plugin is vendored, not depended on: `tools/oxlint/anti-slop/` is a verbatim copy of upstream, held out of prettier, eslint and oxlint so a re-vendor is a clean diff. Changing a rule means editing it there and saying so here.
- **Keep logic in `.ts` modules, not `<script setup>`** — that is what makes it unit-testable and visible to the arch tests, and it is now enforced rather than merely stated (`src/__tests__/architecture/viewLogic.test.ts`: no `switch` at all in a `.vue` file, no `v-if`/`v-else-if` chain comparing one expression against three or more literals, no array literal of mode names or session statuses, no `AsyncResult`). The shapes it pushes you towards, using the timer as the worked example:
  - **A `switch` on a domain union belongs to the feature, once.** `features/timer/labels.ts` is what a timer *reads like* — `modeName`, `modeDescription`, `configSummary`, `humanizeSeconds`, `runPhaseName`, `historyFilterName` — as pure functions over a `translate`, called straight from a template as `modeName(mode, t)`. Keys are built by template literal off `TimerMode`, so a new mode is a compile error there instead of a blank label at runtime; where the compiler cannot reach (`t` satisfies any narrower key type, so `history.<filter>` is not checked), the labels spec walks the vocabulary against the real English catalogue instead. The rule is about the *union*, not about the word `mode`: a `switch` on a phase and a ternary chain on a status were the two that walked past the narrower rule this one replaced.
  - **A flag that changes what a screen renders is a component map.** The setup screen offered its five modes as a `v-if`/`v-else-if` chain, which is a dispatch table with the exhaustiveness check taken out. `features/timer/components/fields/index.ts` is the shape to copy: one component per member, held in a `Record<TimerMode, ModeFields>` the screen renders through `<component :is>`, with `ModeFields` typed as `Component<{ draft: TimerSetupDraft }>` so the props contract is checked rather than assumed. This is **the tree is the variant** (below) applied to a screen rather than to a UI primitive, and it is the same rule.
