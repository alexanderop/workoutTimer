 ...
 * hand-written copy of the default settings row. `stores/timerData.ts` does it
 * once; screens take `{ data, failed, settled }`.
 */
  ...
  it.each(VUE_FILES.map((file) => [file.id, file] as const))(
    ...
    (_id, file) => {
      expect(
        ...
        `${file.id} unwraps an AsyncResult. Take { data, failed, settled } from useSessions / usePresets / useTimerSettings instead — the fallback for a table lives with the table, which is what stopped two screens carrying their own copy of the default settings row.`,
      ).toEqual([])
    },
  )
